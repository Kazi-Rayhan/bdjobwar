...
<script>
    $('#package_select').change(() => {
        ...
        const package = $('#package_select select').find(':selected').val();

        fetch("/ajax/package/" + package)
            .then(response => response.json())
            .then(data => update(data));
    })

    const update = (data) => {
        const is_paid = data.paid;
        if (is_paid) {
            $('#price_toggle .toggle').removeClass(" btn btn-default off");
            $('#price_toggle .toggle').addClass("btn btn-primary");
            $('#price_toggle input[name=is_paid]').prop("checked", true)
        } else {
            $('#price_toggle .toggle').removeClass("btn btn-primary");
            $('#price_toggle .toggle').addClass(" btn btn-default off");
            $('#price_toggle input[name=is_paid]').prop('checked', false);
        }
        const infinite = data.infinite_duration;
        if (infinite) {
            $('#duration_toggle .toggle').removeClass(" btn btn-default off");
            $('#duration_toggle .toggle').addClass("btn btn-primary");
            $('#duration_toggle input[name=infinite_duration]').prop("checked", true)
            $('#expired_date_field input[name=expired_at]').val('');
        } else {
            $('#duration_toggle .toggle').removeClass("btn btn-primary");
            $('#duration_toggle .toggle').addClass(" btn btn-default off");
            $('#duration_toggle input[name=infinite_duration]').prop('checked', false);
            const d = new Date();

            const date = new Date(d.setDate(d.getDate() + data.duration))
            var dateString =
                date.getUTCFullYear() + "-" +
                ("0" + (date.getUTCMonth() + 1)).slice(-2) + "-" +
                ("0" + date.getUTCDate()).slice(-2);
               console.log(dateString);
            // const date = new Date(d.setDate(d.getDate() + data.duration)).toLocaleDateString();
            
         
            $('#expired_date_field input[name=expired_at]').val(dateString);
        }
