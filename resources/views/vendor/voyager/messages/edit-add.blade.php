...
@php
    if (request()->has('batch') && App\Models\Batch::find(request()->batch)) {
        $phones =implode(',',App\Models\Batch::find(request()->batch)->users->pluck('phone')->toArray());
    }else{
        $phones = '';
    }
@endphp
...
<script>
    ...
    var phones = "{{$phones}}";
  document.getElementsByName("numbers")[0].value = phones;
